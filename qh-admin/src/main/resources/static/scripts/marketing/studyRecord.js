app.controller("studyRecordController", function($scope, $http) {
	...
	var finStatementExcelExport=function(){
		window.location.href = "/api/video?numPerPage="+$scope.numPerPage+"&mobile="+$scope.mobile+
		"&videoId="+$scope.videoId+"&date="+changeDate($scope.date)+"&types="+3+"&total="+$scope.total;
	  }
