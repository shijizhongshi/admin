...
submenu="sidebarmenu-system-superAdmin">
		...
		<div class="details-frame">
			<div class="details-frame-content">
				...
				<div class="select-3" style="width: 15%;">
					<span>账号</span> <input type="text" ng-model="username"
						placeholder="输入账号" />

				</div>
				<div class="select-3" style="width: 15%;">
					<span>账号名称</span> <input type="text" ng-model="nickname"
						placeholder="输入账号昵称" />

				</div>

				<div class="select-3" style="width: 15%;">
					<img src="/images/sjk-xl.png" /> <span>账号属性</span> <select
						ng-model="categorys">
			...
			<div class="manage">
				<ul class="show">
					<li style="background: #9DE879;" ng-click="addshow()"><span
						class="glyphicon glyphicon-plus"></span>&nbsp;添加账号</li>
					<li ng-click="update()" style="background: #F9CD33;"><span
						class="glyphicon glyphicon-pencil"></span>&nbsp;修改账号</li>
				...
				<div class="admin-table">
					...
					<table>
						...
						<tr ng-repeat="u in list" ng-class="{'selected':selected==u}"
							ng-click="checkedUserRole(u)">
							<th>{{u.username}}</th>
							<th>{{u.nickname}}</th>
							<th>{{u.password}}</th>
							<th>{{u.category}}</th>
							...
							<th><input type="button" class="btn-lg im-key" value="查看详情"
								style="padding: 3px 10px; margin: 0; background: #F9CD34;"
								ng-click="selectLimits(u.menus)"></th>
						</tr>
					</table>
				</div>
					...
					<form id="myform3" class="ng-pristine ng-valid">
						...
						<div>
							<div style="float: left; width: 49%;">
								<div class="select-2">
									<span>账号昵称<i class="bitian">*</i></span> <input type="text"
										placeholder="请输入昵称" ng-model="userRole.nickname">
								</div>
								...
								<div class="select-2">
									...
									<form id="search">
										...
										<select ng-model="userRole.category"
											ng-options="x for x in category">
											<option ng-selected="status==0" value=0></option>
										</select>
									</form>
								</div>
								<div class="select-2">
									<span>设置账号<i class="bitian">*</i></span> <input type="text"
										class="ng-pristine ng-untouched ng-valid ng-empty"
										placeholder="请设置账号名称" ng-model="userRole.username">
								</div>
								<div class="select-2">
									<span>设置密码<i class="bitian">*</i></span> <input id="userRolePassword" type="password"
										class="ng-pristine ng-untouched ng-valid ng-empty"
										...
										<ul ng-repeat="menu in menus">
											<p class="qxtit" ng-click="unfolf(menu.id)"
												...
												<span class="jian">-</span> <span class="jia">+</span> <input
													type="checkbox" ng-checked="isSelected(menu.names)"
													ng-click="updateSelection($event,menu)"
													style="margin-left: 5px; margin-right: 20px;" />{{menu.names}}
											</p>
											<div class="sj" ng-show="sj==menu.id">
												<li ng-repeat="sub in menu.adminSubMenus"><input
													type="checkbox" ng-checked="isSubSelected(sub.names)"
													ng-click="updateSubSelection($event,sub,menu)" />{{sub.names}}
												</li>
												<p
