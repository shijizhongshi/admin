		...
		.controller(
				...
				function($scope, $http) {
					...
					$scope.chapterBases = function() {
						...
						$http
								.get(
										"/api/course/subclass/courseChapterList",
										{
											"params" : {
												"pageNo" : $scope.pageNo,
					...
					$scope.addChapter = function() {
						$scope.chapter.courseLecturer = $scope.name;
						...
						$http
								.post(
										"/api/course/subclass/courseChapter/saveUpdate",
										$scope.chapter,
										{
											'Content-Type' : 'application/json;charset=UTF-8'
										})
					...
					$scope.deleteChapter = function() {
						if ($scope.chapterId != null) {
							...
							if (confirm("您确定要删出这个课程吗")) {
								$http
										.get(
												"/api/course/subclass/courseChapter/delete",
												{
													"params" : {
					...
					$scope.section = function() {
						if ($scope.chapterId != null) {
							location.href = "/web/course/section?chapterId="
									+ $scope.chapterId + "&chapterName="
									+ $scope.chapter.courseChapterName;
						} else {
							alert("请选中信息~");
						}
					}
					...
					$scope.chapterMoveApi = function(operateType) {
						$http.get("/api/course/subclass/sectionOrders", {
							"params" : {
								"id" : $scope.chapterId,
								"orders" : $scope.chapter.orders,
					...
					$scope.chaptermove = function(types) {
						if ($scope.chapterId != null) {
							if (types == 1) {
								// ///上移
								var index = $scope.chapterlist
										.indexOf($scope.chapter);
								var tmp = angular
										.copy($scope.chapterlist[index - 1]);
								if (index == 0) {
					...
					//radio点击事件
					$scope.radio = function (t) {
						//赋值
						$scope.name = t.name;
						//以下是网上查的 功能是radio只选一个
